...
export default function EditForm({
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        nama_alat: peminjamanDetail.alat.nama_alat,
        nama_pelanggan: peminjamanDetail.lab_pelanggan.nama_pelanggan,
        _method: "PUT",
    });
    ...
    const updatePeminjaman = async (e) => {
        ...
        post(route("lab.peminjaman.update", kode_peminjaman));
    };

    return (
        <form onSubmit={updatePeminjaman} className="form-card">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="form-control">
                    <InputLabel value="Status Peminjaman" />
                    <Dropdown
                        required
                        value={data.status_peminjaman}
                        onChange={(e) => setData("status_peminjaman", e.value)}
                        options={status_peminjamann}
                        optionLabel="status_peminjaman"
                        placeholder="-- Pilih Status Peminjaman --"
                        filter
                        checkmark={true}
                        highlightOnSelect={false}
                        valueTemplate={selectedStatusPeminjamanTemplate}
                        itemTemplate={statusPeminjamanOptionTemplate}
                        className="border-gray"
                    />

                    <InputError message={errors.status_peminjaman} />
                </div>
