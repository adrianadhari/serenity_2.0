...
export default function EditForm({
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        jabatan_penandatangan,
        ruang_lingkup,
        institutions: partnership.institutions.map((inst) => inst.id),
        _method: "PUT",
    });

    const updatePartnership = async (e) => {
        e.preventDefault();

        post(route("kemitraan.update", partnership.kode));
    };

    const handleInstitutionsChange = (e) => {
        const selectedIds = e.value.map((inst) => inst.id);
        setData("institutions", selectedIds);
    };

    return (
        <form
            onSubmit={updatePartnership}
        ...
        >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="form-control">
                    ...
                    {dok_kerjasama ? (
                        <a
                            ...
                            href={`/storage/dokumen-kerjasama/${dok_kerjasama}`}
                        >
                ...
                <div className="form-control">
                    ...
                    {dok_roadmap ? (
                        <a
                            target="__blank"
                            className="text-sm text-blue-500 underline mt-2"
                            href={`/storage/dokumen-kerjasama/${dok_roadmap}`}
                        >
                            Lihat File Sebelumnya
                        </a>
                    ) : (
                ...
                <div className="form-control">
                    ...
                    <MultiSelect
                        required
                        value={institutionOption.filter((inst) =>
                            data.institutions.includes(inst.id)
                        )}
